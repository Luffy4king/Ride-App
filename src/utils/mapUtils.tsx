...
export const getLatLong = async (placeId: string) => {
    console.log("Place ID received in getLatLong:", placeId);
    try {
...
export const reverseGeocode = async (latitude: number, longitude: number) => {
    try {
        const response = await axios.get(
            `https://us1.locationiq.com/v1/reverse.php?key=${process.env.EXPO_PUBLIC_LOCATIONIQ_API_KEY}&lat=${latitude}&lon=${longitude}&format=json`
        );
       
        if (response.data && response.data.address) {
            const address = `${response.data.address.road || ''}, ${response.data.address.city || ''}, ${response.data.address.state || ''}, ${response.data.address.country || ''}`;
            return address.trim();
        } else {
            console.log('Geocoding failed: No address found');
            return "";
        }
    } catch (error) {
...
function extractPlaceData(data: any) {
    return data.map((item: any) => ({
        ...
        latitude: item.lat,
        longitude: item.lon,
        address: item.display_address || item.address, 
        type: item.type
    }));
}
